...
class LookupArgsCallSiteDescriptor extends ArgsCallSiteDescriptor {
     ...
     */
    LookupArgsCallSiteDescriptor(String[] tokenizedName, MethodType methodType, Lookup lookup, Object[] args) {
    ...
    public CallSiteDescriptor dropParameterTypes(int from, int to) {
        return new LookupArgsCallSiteDescriptor(getTokenizedName(), getMethodType().dropParameterTypes(from, to),
                lookup, getArgs());
    }

    @Override
    public CallSiteDescriptor changeParameterType(int num, Class<?> newType) {
        return new LookupArgsCallSiteDescriptor(getTokenizedName(), getMethodType().changeParameterType(num, newType),
                lookup, getArgs());
    }
}
