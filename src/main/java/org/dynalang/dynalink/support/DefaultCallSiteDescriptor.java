...
/**
 * A default, fairly light implementation of a call site descriptor. It has several limitations that still make it
 * suitable for majority of usages: it does not store {@link Lookup} objects but always returns the public lookup from
 * its {@link #getLookup()} method, and it does not support additional static bootstrap arguments, so its
 * {@link #getAdditionalBootstrapArgument(int)} always returns 0. If you need to support either non-public lookup or
...
class DefaultCallSiteDescriptor extends CallSiteDescriptor {
    ...
    /**
     * Checks that the method type has exactly the desired number of arguments, throws an exception if it doesn't.
     *
    ...
    public void assertParameterCount(int count) {
        if(methodType.parameterCount() != count) {
            throw new BootstrapMethodError(getName() + " must have exactly " + count + " parameters");
        }
    }
    ...
    public CallSiteDescriptor dropParameterTypes(int from, int to) {
        return new DefaultCallSiteDescriptor(tokenizedName, methodType.dropParameterTypes(from, to));
    }
    ...
    public CallSiteDescriptor changeParameterType(int num, Class<?> newType) {
        return new DefaultCallSiteDescriptor(tokenizedName, methodType.changeParameterType(num, newType));
    }
