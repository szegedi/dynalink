...
/**
 * A call site descriptor that stores static bootstrap arguments. It does not,
 * however, store {@link Lookup} objects but always returns the public lookup
 * from its {@link #getLookup()} method.
...
class ArgsCallSiteDescriptor extends DefaultCallSiteDescriptor {
    ...
    public CallSiteDescriptor dropParameterTypes(int from, int to) {
        return new ArgsCallSiteDescriptor(getTokenizedName(), getMethodType().dropParameterTypes(
                from, to), args);
    }

    @Override
    public CallSiteDescriptor changeParameterType(int num, Class<?> newType) {
        return new ArgsCallSiteDescriptor(getTokenizedName(), getMethodType().changeParameterType(
                num, newType), args);
    }
}
